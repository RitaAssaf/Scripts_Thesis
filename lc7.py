...
def monitor_and_kill(pid, threshold_gb=6, interval=5):
	...
	try:
		...
		while parent.is_running():
			# Calc7ulate total memory usage (parent + children) in GB
...
if __name__ == '__main__':
	...
	if ilf:
		node_dict,iterations,  time_used, mem_used = run_with_timeout(3600 , ILF, Gp, Gt) #timeout 2 hrs
...
###solve#####################################################################################
		...
		compile()  # generates lc7.xml by default

		xml_filename = f"lc7_{pattern}_{target}_{timestamp}.xml"

		
		shutil.move("lc7.xml", xml_filename)
		...
		if os.path.exists(xml_filename):
			...
			print("Error: lc7.xml file not generated. Check your model definition.")
