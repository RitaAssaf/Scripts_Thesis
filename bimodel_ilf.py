...
from ilf3 import run_bimodel_with_timeout,ILF_bimodel,ILF_prefiltered_domain,run_with_timeout,get_label,hopcroft, build_partial_order_3, build_partial_order_4,hopcroft_multiset,dir_ILF, ILF
...
def domain_n(i, NV_2, mapping): #dom_lc, dom_pos
	if (mapping=='map_pos'):
		if ilf:
			return dom_pos[f'{i}p']
		else:
			return range(NV_2 )
	else:
...
def model(
	...
	pos_A_target: list[Angle]=[]
		...
		map_pos = VarArray(size=pos_NV_pattern, dom=lambda i: domain_n(i, pos_NV_target,'map_pos'))

		map_lc = VarArray(size=lc_NV_pattern, dom=lambda i: domain_n(i, lc_NV_target,'map_rc'))
...
if __name__ == '__main__':
	...
	# pattern='bowtie'
	# target='glc_card_07282025080544'
	# pos_pattern='bowtie'
	# pos_target='gt-07282025080544'
	...
	#Gpos,Grc,Gpos_pattern, Grc_pattern
	...
	# save Gp before labeling it
	Grc_pattern = nx.relabel_nodes(Grc_pattern, {node: f"{node}p" for node in Grc_pattern.nodes()})
	Gpos_pattern = nx.drawing.nx_agraph.read_dot(f'dat/{pos_pattern}.dot')
	Gpos = nx.drawing.nx_agraph.read_dot(f'dat/{pos_target}.dot')
	...
	if ilf:
		dom_lc, dom_pos, iterations, exec_time, peak_memory= run_bimodel_with_timeout(1800 , ILF_bimodel, Gpos,Grc,Gpos_pattern, Grc_pattern, instance) 

	dom_lc = dict(dom_lc)
	dom_pos = dict(dom_pos)
	...
	xml_filename = f"bimodel_ilf__{pattern}_{target_save}_{timestamp}.xml"

	
	shutil.move("bimodel_ilf.xml", xml_filename)
	...
	if os.path.exists(xml_filename):
		...
		print("Error: bimodel_ilf.xml file not generated. Check your model definition.")
