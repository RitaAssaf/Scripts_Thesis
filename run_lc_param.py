...
# python run_lc_param.py --patterns  --targets --output "/home/etud/Bureau/projet/indicators_lines_cols/run_class_b_10012026130521"
...
def handle_error(e,pattern='',target='',ilf=False):
	...
	# Case 2: Process killed by signal
		...
		if sig == signal.SIGINT:
			...
			import_to_excel(f"stop: killed by unknown signal (-{e.returncode})",pattern,target,ilf)
...
def run_lc(patterns,targets, folder):
	...
	common_args_card = [[ "--ilf","--typage", "--ordre", "--card", "--timeout"],
		...
		for target in targets:
			for common_args in common_args_card:
				...
				try:
					...
					handle_error(e,pattern,target,ilf)
...
if __name__ == "__main__":
	...
	# patterns  =["pan"]
	# targets=["glc_card_11252025133942"]
	...
	run_lc(patterns,targets, output_folder)