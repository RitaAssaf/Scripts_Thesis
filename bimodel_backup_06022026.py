...
def model(
	...
	#########Position model#################################################################################################################
		...
		)for column_pattern in set_columns_pattern for t in set_tokens_target 
		],
		)

		#simulation gp> grc

		# [If(And(map_pos[i].among({0,17}) for i in range(2)),Then=map_lc[j].among({k for k in range(2)})) for j in range(2)],
		

		# #simulation grc> gp
		# [If(map_pos[i].among({0,17}) ,Then= And(map_lc[j].among({0,1}) for j in range(2))) for i in range(2)
		# ]
		# )
