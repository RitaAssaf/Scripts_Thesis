...
Usage
-----
python batch_generateur_2.py 5 --nodes 20 --grid 10                      # writes generated_files.xlsx
...
# Regex patterns
#DOT_PATH_RE = re.compile(r"(?P<path>[^\s'\"()]+?\.txt)")
...
def run_generateur_once(nodes: int, grid: int) -> dict:
	...
	for line in stdout.splitlines():
		for m in FILE_PATH_RE.finditer(line):
			...
			# Ignore matrix metrics txt (keeps Excel identical)
			if suffix == ".txt" and "Metrics written" in line:
				continue

			entry = {
				"filename": extract_basename(raw_path),
				"num_nodes": None,
				"num_edges": None,
				"max_degree": None,
				"num_components": None,
...
def add_summary_row(df: pd.DataFrame) -> pd.DataFrame:
	...
	Add a summary row:
	- Combine file names
	- MAX for edges and degree columns
	- MEAN for matrix stats
	"""
	...
	# --- MAX for per-file graph stats ---
	for col in df.columns:
		if col.endswith("_edges") or col.endswith("_degree"):
			summary[col] = df[col].max(skipna=True)
