...
from bimodel import load, load_position, contains
...
def _target_bimodel(queue, func, args, kwargs, memory_limit_mb=None):
	try:
		# Call your function (expects dom_lc, dom_pos)
		dom_lc, dom_pos = func(*args, **kwargs)

		# Track iterations if your function provides them
		iterations = kwargs.get("iterations", None)  # adjust if needed

		# Send the result as a single tuple
		queue.put(("success", (dom_lc, dom_pos), iterations))

	except Exception as e:
		# Send exception info to the main process
		tb = traceback.format_exc()
		queue.put(("error", tb, None))

def run_bimodel_with_timeout(timeout, func, *args, memory_limit_mb=None, **kwargs):
	import multiprocessing, psutil, time

	queue = multiprocessing.Queue()
	process = multiprocessing.Process(target=_target_bimodel, args=(queue, func, args, kwargs, memory_limit_mb))
	process.start()
	...
	try:
		while process.is_alive():
			try:
				mem = proc_ps.memory_info().rss / (1024*1024)  # MB
				peak_memory = max(peak_memory, mem)
			except psutil.NoSuchProcess:
				break
			time.sleep(0.1)
			if time.perf_counter() - start_time > timeout:
				print(f"Function '{func.__name__}' timed out after {timeout}s. Terminating...")
				process.terminate()
				process.join()
				return "TIMEOUT", None, timeout, peak_memory
		...
		if not queue.empty():
			...
			if status == "success":
				dom_lc, dom_pos = result
				print(f"Function completed in {exec_time:.2f}s using {peak_memory:.2f} MB.")
				return dom_lc, dom_pos, iterations, exec_time, peak_memory
		...
		if process.exitcode != 0:
			print(f"Function '{func.__name__}' crashed or was killed.")
		else:
			print("Function exited without returning a result.")
...
def tokens_set(lc_index: int,instance, isPattern= True):
	if isPattern:
...
def ILF_bimodel(Gpos,Grc,Gpos_pattern, Grc_pattern, instance):
	time_used=0
	mem_used=0
	iterations=0
	...
	node_dict = {}
	dom_lc={}
	set_tokens_target=set(range(instance['pos_NV_target']))
	set_rows_target= set([i for i, type in enumerate(instance['lc_type_target'] )])
	set_rows_pattern= set([i for i, type in enumerate(instance['lc_type_pattern'] ) ]) 
	
	dom_lc={i: set_rows_target for i in set_rows_pattern}
	############POS ITERATION"""""""""""""""""""""""""""""""""
	dom_pos,iterations,  time_used, mem_used = run_with_timeout(1800 , dir_ILF, Gpos_pattern,Gpos)
		
	if dom_pos == "TIMEOUT":
		ilf_reached_timeout = True
	elif dom_pos == "ERROR":
		print("ILF crashed or raised exception")
	elif dom_pos is None:
		ilf_empty_domain = True
		return None , None


	########PROPAGATION"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
	for line_pattern in set_rows_pattern:
		for line_target in set_rows_target:
				tokens_line_target=tokens_set(line_target,instance, False)
				if all(set(dom_pos[f'{tp}p']).issubset(set_tokens_target- tokens_line_target) for tp in tokens_set(line_pattern, instance) ):
					dom_lc[line_pattern]=dom_lc[line_pattern]-{line_target}
	######ROWS/COLS ITERATION"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
	dom_lc= ILF_prefiltered_domain(Grc_pattern, Grc, dom_lc)
	return dom_lc, dom_pos


def ILF_prefiltered_domain(Gp, Gt, nodes_domain):
	labeling="degree"
	iterations_ilf=0
	allow_1st_iteration=False
	p = inflect.engine()
	#INITIAL DOMAINS##############################################
	...
	nodes_array = [
	Node(node,"" , [], nodes_domain[int(node.rstrip("p"))], 1) 
	...
	#DOMAIN FILTERING########################################################
	for node in nodes_array:
		for node_j in node._domain.copy():
			if not node._label <= node_dict_name[str(node_j)]._label:
	...
	# ITERATIONS LOOP ##############################################
	for i in range(3, 3): #k= max iterations
		iterations_ilf+=1
...
def load_0(instance, graph_file, id):
...
if __name__ == "__main__":
	...
	Grc_pattern = nx.drawing.nx_agraph.read_dot(f'lcres/{pattern}.dot')
	Grc = nx.drawing.nx_agraph.read_dot(f'lcres/{target}.dot')
	

	Gpos_pattern = nx.drawing.nx_agraph.read_dot(f'{folder}/{pos_pattern}.dot')
	Gpos = nx.drawing.nx_agraph.read_dot(f'{folder}/{pos_target}.dot')
	
	# Transform nodes by appending 'p'
	Gpos_pattern = nx.relabel_nodes(Gpos_pattern, {node: f"{node}p" for node in Gpos_pattern.nodes()})
	Grc_pattern = nx.relabel_nodes(Grc_pattern, {node: f"{node}p" for node in Grc_pattern.nodes()})
	...
	ILF_bimodel(Gpos,Grc,Gpos_pattern, Grc_pattern, instance)