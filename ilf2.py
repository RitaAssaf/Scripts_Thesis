	...
	try:
		while process.is_alive():
			...
			if time.perf_counter() - start_time > timeout:
				...
				process.terminate()
				process.join()
				return None,None, timeout, peak_memory
		...
		if not queue.empty():
			...
			if status == "success":
				...
				print(result)
				return None,None, exec_time, peak_memory
		...
		return None,None, time.perf_counter() - start_time, peak_memory
...
def ILF(Gp, Gt, posp={}, post={}, instance=[]):
	...
	# ITERATIONS LOOP ##############################################
	for i in range(2, 100): #k=100 max iterations
		iterations_ilf+=1
...
if __name__ == "__main__":
	...
	result = run_with_timeout(2000, dir_ILF, Gp, Gt, posp, post)
