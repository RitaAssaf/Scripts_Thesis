...
# 		print(f"Error saving excel file: {e}, {clean_output}")
# 		sys.exit(1)
...
# def import_to_csv(solver_output):
# 	# Step 1: Clean ANSI escape sequences
# 	ansi_escape = re.compile(r'\x1B\[[0-?]*[ -/]*[@-~]')
# 	clean_output = ansi_escape.sub('', solver_output)
...
# 	timestamp = datetime.now().strftime("%d%m%Y%H%M%S")
# 	file_name = os.path.join(output_folder, f"indicators_{pattern}_{target}_{timestamp}.csv")
		
# 	try:
# 		#raise MemoryError("Simulated OOM for testing after solver run")
...
# 		print(f"Data has been written to {file_name} ")
# 	except MemoryError:
# 		print(f"MemoryError: Not enough memory to save CSV file., {clean_output}")
# 		sys.exit(1)
# 	except Exception as e:
# 		print(f"Error saving CSV file: {e}, {clean_output}")
# 		sys.exit(1)
...
def monitor_and_kill(pid, threshold_gb=6, interval=5):
	...
	try:
		...
		while parent.is_running():
			# Calculate total memory usage (parent + children) in GB
...
if __name__ == '__main__':
	...
	if ilf_reached_timeout:
		...
		compile()  # generates lc5.xml by default

		xml_filename = f"lc5_{pattern}_{target}_{timestamp}.xml"

		
		shutil.move("lc5.xml", xml_filename)
		...
		if os.path.exists(xml_filename):
			...
			try:
				...
				pids_to_watch = get_process_tree_pids(solver_pid)
				monitor = threading.Thread(
				target=monitor_and_kill_multiple,
				args=(solver_pid, MEMORY_LIMIT_GB * 0.8, timeout, 0.2),
				daemon=True
				)
				...
				# Decide the stop reason ONCE, from events
				if MEMORY_EVENT.is_set():
					...
					if process.returncode == 0:
						...
						if stderr and "OutOfMemoryError" in stderr:
							import_to_excel('stop: ACE_OOM')
						else:
				...
				import_to_excel('stop: ACE_MEM_LIMIT_excel')
