...
def analyze_matrix_distribution(A, output_file="matrix_metrics.txt"):
	...
	#max cardinality of rows/cols
	max_card=max(np.max(A.sum(axis=1)), np.max(A.sum(axis=0)))
	...
	with open(output_file, "w") as f:
		...
		f.write(f"max_card: {max_card}\n")

	print(f"✅ Metrics written to '{output_file}'")

	return m, n, N, density, row_var, col_var, i_mean, j_mean, spread, entropy, max_card
...
def main():
	...
	m, n, N, density, row_var, col_var, i_mean, j_mean , spread, entropy,max_card=analyze_matrix_distribution(positions_to_study)
...
############### création de l'hypergraphe de Stell avec uniquement les lignes et colonnes complètes
	...
	affichage={'GP':True,'Equivalent':True,'glc_card': False,'GT':False,'GLCRed':False,'GLCall':False,'HS':False,'GLCT': False,'imageGRed': False, 'imageGLCall': False }

	# deux graphes de positions (réduit et avec transitivité
	if affichage['GP']:
