...
def _target(queue, func, args, kwargs, memory_limit_mb=None):
	try:
		...
		queue.put(("success", result, exec_time, memory_usage))
	except MemoryError:
		queue.put(("error", "MemoryError: Function exceeded memory limit", None, None))
	except Exception:
		queue.put(("error", traceback.format_exc(), None, None))
...
def run_with_timeout(timeout, func, *args, memory_limit_mb=None, **kwargs):
	...
	if process.is_alive():
		...
		process.terminate()
		process.join()
		return None, exec_time, memory

	if not queue.empty():
		status, result, exec_time, memory = queue.get()
		if status == "success":
			print(f"Function completed in {exec_time:.2f} seconds using {memory:.2f} MB memory.")
	...
	# If process exited but no result is in queue
...
# def _target(queue, func, args, kwargs):
# 	try:
# 		result = func(*args, **kwargs)
...
# 	queue = multiprocessing.Queue()
# 	process = multiprocessing.Process(target=_target, args=(queue, func, args, kwargs))
# 	process.start()
# 	process.join(timeout)
