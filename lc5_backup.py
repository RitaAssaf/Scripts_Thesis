...
def import_to_excel(solver_output):
	...
	headers = ["Pattern", "Target", "ILF", "Order", "Typing","Card","Run", "Dpts", "Effs", "Fails", "Wrgs", "Wck", "Ngds", "Sols", "ilftime s", "ilfmemo MB"]
	ws.append(headers)
	...
	for match in matches:
		...
		row = [pattern, target, ilf, ordre, typage, card,f"run{run_count}", dpts, effs, fails, wrgs, wck, ngds, sols, time_used, mem_used]
		ws.append(row)
		run_count += 1
	...
	# If no matches were found, 
		...
		if not matches:  # Ensures the message only appears when the list is empty
			...
			headers = ["Pattern", "Target", "ILF", "Order", "Typing","Card","Run", "Dpts", "Effs", "Fails", "Wrgs", "Wck", "Ngds", "Sols", "Stop", "CPU", "Mem", "UNSAT", "WrongDec", "FoundSols", "Complete", "ilftime", "ilfmemo"]
			ws.append(headers)
...
if __name__ == '__main__':
	...
	# pattern='bowtie'
	# target='glc_card_06252025113405'
	# ilf = True
	# ordre = True
	# typage = True
	# card= True
	...
	ilf_reached_timeout= False
	if ilf:
		node_list,  time_used, mem_used = run_with_timeout(10800 , ILF, Gp, Gt) #timeout 3 hrs
	...
	if ilf_reached_timeout:
		...
		if os.path.exists("lc5.xml"):
			if(enable_timeout):
				...
				command = ["java", "-jar", ace_jar_path, "lc5.xml", "-s=all",   "-trace", "-ev"]
